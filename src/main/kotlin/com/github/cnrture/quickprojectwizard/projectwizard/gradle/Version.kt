...
object Versions {
    private val _versionListState = MutableStateFlow(
        mutableMapOf(
            "agp" to "8.9.3",
            "kotlin" to "2.1.21",
            "core-ktx" to "1.16.0",
    ...
    val versionListState = _versionListState.asStateFlow()
    val versionList get() = _versionListState.value

    fun updateVersions(newVersions: Map<String, String>) {
        val current = _versionListState.value.toMutableMap()
        current.putAll(newVersions)
        _versionListState.value = current
    }
}
...
sealed class Version(val name: String, val value: String) {
    ...
    data object Ktorfit : Version("ktorfit", Versions.versionList["ktorfit"] ?: "2.7.2")
